...
<form method="post" action="<%=CONTROLLER%>/Boleto/Modificar"  
name="TarifasReservas"   onSubmit="return validar(this)" 
>

<table width="100%" align="CENTER">
<tr class="form1">
<th colspan="8" >Modificar pagos</th>
</tr>
<tr class="form1">
...
<%=f.format(rtvalor*descuento+rtq+rtiva*descuento+rttasa+rtseguro+msc )%>
</span>
<input name="rttotal<%=i%>" value="<%=rtvalor+rtiva+rttasa+rtseguro+msc%>" 
type="hidden"  >
</td>
...
type="hidden"  >
<%------------
<input name="descuento<%=i%>" value="<%=reserva.getDescuento()%>"  onchange="cambiadescuento(this.form,'<%=i%>','<%=j%>')"
type="text" size="2" maxlength="4" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" >
----------%>
</td>
...
<%=f.format(reserva.getValor()*descuento+reserva.getIva()*descuento+reserva.getSeguro()+reserva.getTasa())%>
<input name="total<%=i%><%=j%>" value="<%=reserva.getValor()+reserva.getIva()+reserva.getSeguro()+reserva.getTasa()%>" 
type="hidden"  >
</span>
</td>
...
<%-------------------------
onchange="cambiacliente(this.form.agencias.selectedIndex)">
<select name="agencias" onchange="cambiacliente(this.form.agencias.options[this.form.agencias.selectedIndex].value)">
comisionesf(10,3,'true','comisiones','tabla2')">
----------------%>
...
<div id="comisiones" style="position:relative;  overflow: visible"> 
</div>
-----------%>
...
if (listfp != null) {
      ...
      while (itlistfp.hasNext()) {
          ...
          {
              pta=f.format(forma.getValor());//-tcomis);
          ...
          if ((efectivo==null) ||  (contacto.getCodigopta()>0))
          {
              efectivo="0";
              obsefectivo="";
          }
          if ((canje==null) ||  (contacto.getCodigopta()>0))
          {
              canje="0";
             proveedores=null;
          }
          if ((cheque==null) ||  (contacto.getCodigopta()>0))
          {
              cheque="0";
              obsch="";
              documentoch="";
          }
          if ((mpd==null)||  (contacto.getCodigopta()>0))
          {
              mpd="0";
              obsmpd="";
              documentompd="";
          }
          if ((cr==null) ||  (contacto.getCodigopta()>0))
          {
              cr="0";
              obscr="";
              documentocr="";
          ...
          {
              //pta="0";
              obspta=contacto.getNombrepta();
              documentopta=String.valueOf(contacto.getNumeropta());
          }
          if ((otros==null) ||  (contacto.getCodigopta()>0))
          {
              otros="0";
              obsotros="";
              documentootros="";
...
type="<%=tipotexto%>" size="8" maxlength="10" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" >
</td>
<td colspan="2">
<input name="obsefectivo" value="<%=obsefectivo%>"  
type="<%=tipotexto%>" size="30">
</td>
</tr>
<tr class="form1">
...
type="<%=tipotexto%>" size="10">

<td colspan="1">
<input name="obsch" value="<%=obsch%>"  
type="<%=tipotexto%>" size="20">
</td>
</tr>
<tr class="form1">
<th colspan="3" >MPD</th>
...
<tr>
<td>
<input name="pta" value="<%=pta%>"    onchange="pagados(this.form)" 
type="<%=tipopta%>" size="8" maxlength="10" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" >
</td>
<td colspan="1">
<input name="documentopta"  value="<%=documentopta%>"  
type="<%=tipopta%>" size="10">

<td colspan="1">
<input name="obspta"  value="<%=obspta%>"  
type="<%=tipopta%>" size="20">
</td>
</tr>
<tr class="form1">
...
type="<%=tipotexto%>" size="8" maxlength="20" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" >
</td>
<td>
<select name='Tc1'>
<%
...
-----------------%>
function pta(formulario){
efe=eval("formulario.pta");
<% if (!tipopta.equalsIgnoreCase("TEXT"))
    totalpagado=0;%>
efe.value=<%=totalpagado%>;
}
function cambiasc(formulario,i,j){
 ...
 function cambiadescuento(formulario,i,j)
{
 ...
 for (k=1;k<=<%=j%>;k++)
 {
    ...
    var iva=parseFloat(valordesc)*.12;
    tttvalor+=valordesc;
    tttiva+=iva;
    //alert(capa);
...
var valor=0;
    for (i=1;i<=<%=i%>;i++)
    {
     ...
     Valor="rrValor"+i;
     Iva="rrIva"+i; 
     var xx=document.getElementById(Valor).innerHTML;
     
     //alert(Valor+ "-----"+parseFloat(document.getElementById(Valor).innerHTML)+"........"+xx);
    ...
    <% if (usuario.getAcceso()==1)
    {%>
    document.getElementById("saldopesos").innerHTML=Math.round((total-pag-<%=tcomis%>)*<%=cambio%> * 100) / 100 ;
    <%}%>
}
...
<td align="center">
 ...
 <%---------------------------------
 <td align="center">
 <input TYPE="submit"    NAME="Reserva" value="Modificar Reserva" onclick="salirsi();">
 </td>
 ------------------------%>
